 ...
 */
@Slf4j
public class MediaChannelImportListener implements ReadListener<MediaChannelImport> {
     ...
     * 缓存数据
     */
    private List<MediaChannelImport> cachedList = new ArrayList<>(CACHE_SIZE);


    public MediaChannelImportListener(Long projectId, String taskId, DataImportService service) {
    ...
    @Override
    public void invoke(MediaChannelImport data, AnalysisContext context) {
        log.info("解析第" + context.readRowHolder().getRowIndex() + "行数据:{}", data);
        data.setCreateTime(new Date());
        data.setTaskId(taskId);
        data.setProjectId(projectId);
        data.setSeq(context.readRowHolder().getRowIndex());
        if (data.getModelName().toLowerCase().contains("offline")) {
            data.setLine("offline");
        } else {
            data.setLine("online");
        }
        cachedList.add(data);
    ...
    private void saveData() {
        try {
            ...
            int i = service.saveMediaChannel(cachedList);
